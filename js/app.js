(() => {
    ...
    class Popup {
        ...
        _openToHash() {
            ...
            if (popupElement) {
                this.targetOpen.selector = `#${classInHash}`;
                this.targetOpen.element = popupElement;
                const button = document.querySelector(`[${this.options.attributeOpenButton}="#${classInHash}"]`);
                if (button && button.hasAttribute(this.options.youtubeAttribute)) this.youTubeCode = button.getAttribute(this.options.youtubeAttribute);
    ...
    function initSliders() {
        ...
        if (document.querySelector(".products-slider__body")) new Swiper(".products-slider__body", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
...
***************************************************************************** */
    ...
    const gallerySingle = document.querySelectorAll("[data-gallery-single]");
    if (gallerySingle.length) {
        let galleyItems = [];
        gallerySingle.forEach((gallery => {
            galleyItems.push({
                gallery,
                galleryClass: lightgallery_es5(gallery, {
                    licenseKey: "7EC452A9-0CFD441C-BD984C7C-17C8456E",
    ...
    function documentActions(e) {
        const targetElement = e.target;
        if (targetElement.closest(".search-btn-js")) {
            const searchBlock = document.querySelector(".search-block");
            searchBlock.classList.toggle("_show");
        }
    }
    if (document.querySelector(".form_calc")) {
        function groutCalculator() {
            const form = document.querySelector(".form_calc");
            if (!form) return;
            const dlinaInput = form.querySelector("#dlina");
            const tolwinaInput = form.querySelector("#tolwina");
            const shirinaPlitkiInput = form.querySelector("#shirina_plitki");
            const shirinaShwaInput = form.querySelector("#shirina_shwa");
            const ploshadInput = form.querySelector("#ploshad");
            const resultBlock = form.querySelector(".js-result");
            const inputs = [ dlinaInput, tolwinaInput, shirinaPlitkiInput, shirinaShwaInput, ploshadInput ];
            ...
            function calculate() {
                const dlina = parseInput(dlinaInput.value);
                const tolwina = parseInput(tolwinaInput.value);
                const shirinaPlitki = parseInput(shirinaPlitkiInput.value);
                const shirinaShwa = parseInput(shirinaShwaInput.value);
                const ploshad = parseInput(ploshadInput.value);
                const currentResult = parseInput(resultBlock.textContent);
                if (dlina && tolwina && shirinaPlitki && shirinaShwa && ploshad) {
                    const newResult = (dlina + shirinaPlitki) / (dlina * shirinaPlitki) * tolwina * shirinaShwa * 1.6 * ploshad;
                    animateResult(currentResult, newResult, 500);
                } else animateResult(currentResult, 0, 500);
            }
            function onInput(e) {
                const input = e.target;
                const originalLength = input.value.length;
                const cursorPosition = input.selectionStart;
                let newValue = formatInputValue(input.value);
            ...
            function animateResult(from, to, duration = 500) {
                const start = performance.now();
                function animate(time) {
                    const progress = Math.min((time - start) / duration, 1);
                    const current = from + (to - from) * progress;
                    resultBlock.textContent = formatResult(current);
